...
__global__ void find_collisions(volatile char* collision) {
    ...
    do
    {
        while ( random_index < NUM_RANDOMS)
        {
            ...
            for (int c = 1; c < CHAR_SET_SIZE; ++c) {
                // temporary append byte to try
                local_collision[local_collision_size - 1] = randoms[random_index] ? randoms[random_index] : (uint8_t)1; // no premature null terminators
